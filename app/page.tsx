...
const calculatePersona = (riskScore: number, discScore: number) => {
  ...
  const maxDist = 14.14; // ระยะไกลสุดที่เป็นไปได้ในกราฟ 10x10 (รูทของ 10^2 + 10^2)
...
export default function Home() {
  ...
  const handleStart = () => {
    ...
    const pickedL1 = shuffleArray(l1Questions).slice(0, 3);
    const pickedL2 = shuffleArray(l2Questions).slice(0, 4);
    const pickedL3 = shuffleArray(l3Questions).slice(0, 3);

    setActiveScenarios([...pickedL1, ...pickedL2, ...pickedL3]);
    setAnswers([]); 
    setCurrentIndex(0);
